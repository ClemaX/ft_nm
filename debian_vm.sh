...
iso_url="https://sourceforge.net/projects/osboxes/files/v/vb/14-D-b/11/Server/64bit.7z/download"

vm_dir="$PWD"

vm_os="Linux_64"
vm_image="$vm_dir/Debian"
vm_name="Debian"
vm_ram="1024"
vm_vram="0"
vm_gfx="none"
...
vm_up()
{
	...
	then
		...
		pushd "$(dirname "$vm_image")"
			while ! sha256sum --check sha256sums
			do
				echo "Fetching '$vm_image'..."

				curl --insecure -L -O -C - "$iso_url" --output "$vm_image.7z"
				p7zip e "$vm_image.7z"
				mv *.vdi "$vm_image.vdi"
			done
		popd
